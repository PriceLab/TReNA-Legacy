#------------------------------------------------------------------------------------------------------------------------
.FlamSolver <- setClass ("FlamSolver", contains="Solver")
#------------------------------------------------------------------------------------------------------------------------
FlamSolver <- function(mtx.assay=matrix(), quiet=TRUE)
{
    obj <- .FlamSolver(Solver(mtx.assay=mtx.assay, quiet=quiet))

    obj

} # TReNA, the constructor
#------------------------------------------------------------------------------------------------------------------------
setMethod("getSolverName", "FlamSolver",
...
#----------------------------------------------------------------------------------------------------
setMethod("run", "FlamSolver",
######### Alter file below here, it's currently just Lasso ################
  function (obj, target.gene, tfs, tf.weights=rep(1,length(tfs)), extraArgs=list()){
   ...
   if("keep.metrics" %in% names(extraArgs))
    ...
    if(length(deleters) > 0){
     ...
     if(!obj@quiet)
	message(sprintf("FlamSolver removing target.gene from candidate regulators: %s", target.gene))
       }
...
#----------------------------------------------------------------------------------------------------
setMethod("trainModel", "FlamSolver",
...
#----------------------------------------------------------------------------------------------------
setMethod("predictFromModel", "FlamSolver",
...
setMethod("rescalePredictorWeights", "FlamSolver",
